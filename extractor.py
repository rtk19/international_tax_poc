...
def find_ordered_header_columns(row_values: list[Any], expected_headers: list[str]) -> list[int] | None:
    """Find expected headers in-order while allowing blank/merged spacer columns."""
    ...
    for header in expected:
        ...
        for idx in range(start_col, len(row_values)):
            current = normalize_header(row_values[idx])
            if not current:
                continue
            if current == header:
                matched_col = idx + 1
...
def find_header_row_by_exact_headers(sheet: Worksheet, headers: list[str]) -> tuple[int, list[int]]:
    ...
    for row in range(1, max_row + 1):
        ...
        col_indexes = find_ordered_header_columns(row_values, headers)
        if col_indexes:
            return row, col_indexes

    raise ExtractionError(f"Could not find table headers (ordered match): {headers}")
